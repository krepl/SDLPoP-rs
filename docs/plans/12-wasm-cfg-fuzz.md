...
- [x] Quicksave cross-compatibility — the tests above only prove Rust's own read/write
      agree with each other, not that Rust matches the *real* file format the C oracle
      produces. Closed with a committed fixture: `doc/test-fixtures/quicksave_c_oracle.sav`,
      a real `QUICKSAVE.SAV` generated by the actual C `quick_save()` via
      `scripts/gen_quicksave_fixture.sh` + `src/test_quicksave_fixture.c` (a throwaway
      executable linking the same sources as the oracle, swapping in a minimal `main()` —
      does *not* touch the pinned `src/CMakeLists.txt`/`src/Makefile`, since `quick_save()`
      has no SDL calls and needs no game-startup sequence). `quicksave_reads_fixture_written_by_c_oracle`
      (`seg000.rs`) loads that committed fixture and asserts Rust reads back the exact
      values the C oracle wrote. Same idea in reverse (a Rust-written save loading correctly
      in the C binary) isn't covered — lower value, since the practical question is whether
      *existing player saves* load in the ported version, not whether new Rust-written ones
      would load in the original.
