...
### Scripted keyboard input (`POPTRACE_INPUT`)

`load_scripted_input`/`inject_scripted_input` (`rust/src/seg009.rs`) push synthetic `SDL_KEYDOWN`/`SDL_KEYUP` events onto the real SDL event queue at scripted tick numbers, read from a `POPTRACE_INPUT`-named file (`scripts/scripted_inputs/*.txt`, format: `<tick> <key> <down|up>` per line). This exercises the *live* input path (`process_events()` → `key_states` → `control_kid()`) end-to-end, unlike `validate` mode's canned replay input — see `scripts/gameplay_smoke_test.sh`'s header for the coverage gap this closes. Reads via the shared `getenv`/`fopen`-family primitives (not `std::env`/`std::fs`), so the exact same script works unchanged on wasm too via `web/headless.mjs`'s `runHeadlessScriptedInput` (`wasm_setenv`/`preload_file` instead of a real env var/file) — see `scripts/wasm_menu_smoke_test.mjs` for a full example. A script that opens the pause menu and never closes it again is a valid, useful test (see `scripts/scripted_inputs/open_menu.txt`'s header for why a scripted close isn't feasible): the game just hangs waiting for real input, same as it would with a real idle keyboard, and the test harness treats "ran for a bounded timeout without crashing" as the pass condition — native via `timeout`'s exit code, wasm via a **Node-side** timer racing the (necessarily un-awaited) `page.evaluate()` call, since wasm is single-threaded and a JS-side timer can never fire while the page is synchronously blocked inside the wasm call.
...
`web/headless.mjs` also exports `runHeadlessScriptedInput` — the live-scripted-keyboard-input counterpart to `runHeadlessReplay`, used by `cargo xtask wasm-menu-smoke-test` (`scripts/wasm_menu_smoke_test.mjs`) to confirm opening the pause menu doesn't crash the wasm build. See "Scripted keyboard input" above for the shared mechanism and why the pass condition is a Node-side timeout race, not a return value. Requires the same `npm install`/`cargo xtask wasm-build` setup as the replay tools above; also not wired into `harness`/`verify` for the same dependency-weight reason.
...
- **Worktree agents must start from the correct base (mandatory).** `origin/master` on this repo is chronically far behind local `master` (rarely pushed) — `isolation: "worktree"` Agent calls and `EnterWorktree` default to branching from `origin/<default-branch>`, which silently gives agents a stale base missing recent commits/fixes. This is fixed structurally via `.claude/settings.json`'s `worktree.baseRef: "head"` (branches from local HEAD instead) — **do not remove that setting**. As defense in depth, every worktree-agent prompt should still state the exact expected base commit and instruct the agent to verify with `git log --oneline -1` and `git reset --hard <sha>` if it doesn't match, since a misconfigured environment or a future settings change could silently reintroduce the stale-base problem. A batch of six agents once needed manual merge-conflict resolution from exactly this failure mode — see memory `feedback_worktree_stale_origin_base` for the full incident.
